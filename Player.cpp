	...
	while(!valid_pokeball) {	// Se selecciona la pokeball a usar
		...
		switch(pokeball) {
			...
			case 4:
				...
				break;
			default:
				std::cout << "Pokeball elegida no v·lida" << std::endl;
				break;
		}
	}
...
void Player::giveObject() {
	...
	while(giving) {
		...
		std::cin >> pokemon_team;									// Indico a cu·l se lo doy
		std::cout << "Indique el numero del objeto que le voy a dar: ";
		std::cin >> object;											// Indico quÅEobjecto le doy
...
void Player::selectActive() {
	...
	while(!valid_pokemon) {
		...
		if(chosed_pokemon <= pokemon_quantity && chosed_pokemon > 0 ) {	// Verifico que el n˙mero este dentro del rango
			if(!pokemons.at(team.at(chosed_pokemon - 1)).isDefeated()) {			// Verifico que el Pokemon elegido no este debilitado
				...
				std::cout << std::endl;
				pokemons.at(active_pokemon).modifierRefresh();		// Elimino cualquier modificador de estad˙êticas que le haya quedado
		...
		} else std::cout << "Pokemon no v·lido" << std::endl;
	}
}
...
void battle(std::vector<Player> &players) {
	...
	std::vector<std::vector<bool>> player_battle_states {};	// Indica los estados: 1=Viento Af˙ã
	...
	for(auto &player: players) {	// Indico que jugadores participan y sus respectivos adversarios
		if(player.defeated) {	// Si el jugador no participa
			...
			enemy_pokemons.push_back(Pokemon());							// Agrego un Pokemon enemigo "dummy" por temas de llenar el vector para la sincronizaciÛn
		} else {
			...
			player.selectActive();											// ElecciÛn de Pokemon activo de cada jugador que pueda pelear
		...
		for(size_t i = 0; i < players.size(); i++) {	// Se descenlazan las acciones de cada jugador con su respectivo enemigo
			if(player_battling.at(i)) {		// Se verifica que el jugador este luchando
				...
				while(!valid_move) {	// Selecciono movimiento del Pokemon enemigo
					...
					std::cin >> chosed_move;
					if(chosed_move <= moves_quantity && chosed_move > 0 ) {			// Se verifica que la selecciÛn sea v·lida
						valid_move = true;
						enemy_chosed_move = enemy_pokemons.at(i).getMove(chosed_move);	// De ser v·lida, se alamacena el movimiento a usar por el pokemon enemigo
						std::cout << "El pokemon salvaje ha usado ";
						enemy_pokemons.at(i).displayMove(chosed_move);
					} else std::cout << "Movimiento no v·lido" << std::endl;
				}
				valid_move = false;

				while(!valid_action) {	// Selecciono la acciÛn del jugador
					std::cout << "Elija la acciÛn a ejecutar del jugador " << players.at(i).name << ": " << std::endl
							  << std::setw(12) << std::right << "1 Movimiento"
							  << std::setw(12) << std::right << "2 Capturar"
							  << std::setw(12) << std::right << "3 Cambiar"
							  // Falta agregar opc˙ã de usar objetos
							  << std::endl;
					std::cin  >> chosed_action;
					if(chosed_action <= 3 && chosed_action > 0 )
						valid_action = true;
					else
						std::cout << "AcciÛn no v·lido" << std::endl;
				}
				valid_action = false;

				switch(chosed_action) {	// Depende a la acciÛn, se prosigue
					case 1:	// SelecciÛn de usar movimiento
						// Se podr˙} integrar esto como una funciÛn
					...
					case 2:	// SelecciÛn de usar pokeball
						if(players.at(i).capturePokemon(enemy_pokemons.at(i)))
							player_battling.at(i) = false;	// ElecciÛn de Pokeball y ejecuciÛn de captura
						else
							enemy_pokemons.at(i).useMove(players.at(i).pokemons.at(players.at(i).active_pokemon), enemy_chosed_move);
						break;

					case 3:	// SelecciÛn de cambiar pokemon
						if(enemy_chosed_move == "Persecucion")											// Verifica si se utilizÅEpersecucion
				...
				if(player_battling.at(i) == true) {	// Se verifica que el combate no haya acabado por la captura del enemigo
					if(getPriority(player_chosed_move) > getPriority(enemy_chosed_move) || (!(getPriority(player_chosed_move) < getPriority(enemy_chosed_move)) && players.at(i).pokemons.at(players.at(i).active_pokemon).getVelocity() >= enemy_pokemons.at(i).getVelocity())) {	// Se descenlazan daÒo por estados en el orden de los movimientos
						if(players.at(i).pokemons.at(players.at(i).active_pokemon).isSeeded() && !enemy_pokemons.at(i).isDefeated())		// Se descuenta el daÒo de estados
							enemy_pokemons.at(i).cureDamage(players.at(i).pokemons.at(players.at(i).active_pokemon).receiveDamage().damage);// Al no indicar valor de daÒo (por default 0) el mÈtodo verificarÅEsi el pokemon tiene alg˙n estado y calcularÅEel daÒo
						else
							players.at(i).pokemons.at(players.at(i).active_pokemon).receiveDamage();
						if(enemy_pokemons.at(i).isSeeded() && !players.at(i).pokemons.at(players.at(i).active_pokemon).isDefeated())		// Se descuenta el daÒo de estados
							players.at(i).pokemons.at(players.at(i).active_pokemon).cureDamage(enemy_pokemons.at(i).receiveDamage().damage);// Al no indicar valor de daÒo (por default 0) el mÈtodo verificarÅEsi el pokemon tiene alg˙n estado y calcularÅEel daÒo
						else
				...
				// Se descuenta el daÒo de climas
...
void pokemonCenter(std::vector<Player> &players) {
	char answer {};
	for(auto &player: players) {	// Ejecuto acciÛn en cada jugador
