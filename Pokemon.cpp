...
void Pokemon::passCampusStatus(Pokemon &last_pokemon) {
	...
	if(toxic_spikes || toxic_spikes_plus) {	// Se activa el efecto de Púas Tóxicas
		bool immune {false};
		bool poison {false};
		bool flying {false};
		...
		if(info.hability == 17)		// Inmunidad por habilidad Inmunidad
			immune = true;
		if(info.hability == 26) {	// Inmunidad por habilidad Levitación
			immune = true;
			levitation = false;
		}
		...
		if(poison && !flying && !levitation) {	// Despeja las Púas Tóxicas
			toxic_spikes = false;
...
void Pokemon::newMove(bool newPokemon, bool evolutionMove) {
	...
	for(auto j = temp_level; j < end_level + 1; j++) {	// Este for es para contemplar la creación de un nuevo Pokemon
		for(auto i = 0; i < 3; i++ ) {		// Contempla hasta 3 movimientos nuevos por nivel
			if(it != info.moves.end()) {		// Verifico que si haya algún movimiento que se gana en ese nivel
				if(it->level == j) {	// Verifico si se aprende en este nivel (esta comprobación está más que nada por el 2do movimiento
...
void Pokemon::evolve() {
	...
	newMove(false, true);	// Para aprender movimientos de Evolución
}
...
void Pokemon::counterUp() {
	...
	if(light_screen_counter == 5) {// Se acabó el efecto de Pantalla de Luz
		light_screen = false;
		light_screen_counter = 0;
	}

	if(safeguard)				// Aumentó contador de Velo Sagrado
		safeguard_counter += 1;

	if(safeguard_counter == 5) {// Se acabó el efecto de Velo Sagrado
		safeguard = false;
		safeguard_counter = 0;
	}

	if(tailwind)				// Aumentó contador de Viento Afín
		tailwind_counter += 1;

	if(tailwind_counter == 4) {	// Se acabó el efecto de Viento Afín
		tailwind = false;
		tailwind_counter = 0;
	}
}
...
void Pokemon::useMove(Pokemon &objetive, std::string move) {
	float accuracy_dice {};				// Valor del dado de 1-20, siendo 1 críico
	...
	if(confused && !failed && !(state == "Dormido")) {	// Verifico si se autogolpea por la Confusión y si se libera de la Confusi
	...
	if(!(state == "Dormido") && !failed) {			// Solo se realizar・el movimiento si el Pokemon está despierto
...
void Pokemon::ejecuteMove(Pokemon &objetive, Move &move_info) {
	float accuracy_dice {10};			// Valor del dado de 1-20, siendo 1 crítico; se pone en 10 por movimientos de daño como Golpe A駻eo que nunca fallan
	float accuracy_hability_mod {1.0};	// Modificador de la Precisión porducto de una habilidad
	...
	if(move_info.objetive == 0 || move_info.objetive == 3) { // En el caso que el movimiento sea para el mismo usuario, ya se procede con el efecto
		...
		if(move_info.effect_1 == "Retroceso") {
			std::cout << info.name << " ha sufrido daño de retroceso" << std::endl;
		...
		if(move_info.contact && objetive.info.hability == 9 && state == "None") {	// Habilidad Electricidad Estática
			std::cout << info.name << " lanza el dado para verificar si es afectado por la habilidad Electricidad Estática: ";
			std::cin >> accuracy_dice;
			if(accuracy_dice > 14) {
				state = "Paralizado";
				std::cout << info.name << " ahora está Paralizado" << std::endl;
			}
		}

		if(move_info.contact && objetive.info.hability == 38 && state == "None") {	// Habilidad Punto Tóxico
			std::cout << info.name << " lanza el dado para verificar si es afectado por la habilidad Punto Tóxico: ";
			std::cin >> accuracy_dice;
			if(accuracy_dice > 14) {
				state = "Envenenado";
				std::cout << info.name << " ahora está Envenenado" << std::endl;
			}
		}
...
void Pokemon::receiveEffect(Move &move_info) {
	...
	if(move_info.effect_1 == "Estadistica") {	// Efectos que modifican estadisticas
		for(auto effect_2: move_info.effect_2) {
			...
			} else if(effect_2 == "Critico") {		// Modifica el índice de golpe crítico
