...
/*
 * El objetivo de esta versión es el de funcionar como un hibrido entre el
 * juego de mesa para realizar las cuentas. No se busca que guie a los
 * jugadores totalmente en su viaje, sino acompañarlos en todos los calculos
...
void newGame() {
	...
	for(int i = 0; i < players_quantity; i++) {				// Se le asigna a cada jugador un Pokemon inicial
		...
		// Esto solo estEpara el debuggeo
...
void newZone() {
	...
	bool event_valid {false};	// Para verificar que se eligiEun evento válido
	...
	while(!event_valid) {		// Selecciono un tipo de zona
		...
		if(event == 1 || event == 2) {
			event_valid = true;
			std::cout << "Evento válido" << std::endl;
		} else std::cout << "Evento no válido" << std::endl;
	}
...
void newEvent() {
	...
	while(!event_valid) {		// Selecciono un tipo de evento
		std::cout << "Indique el número de evento: " << std::endl
